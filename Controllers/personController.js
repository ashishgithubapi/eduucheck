...
module.exports.Userreg = function(req,res){
    ...
    if(req.body.gst_base64data.length>0){
        ...
        Jimp.read(buffer, (err, res2) => {if (err) throw new Error(err); res2.quality(100).write('/app/' +req.body.gst_filename); });
    ...
    if(req.body.doc_base64data.length>0){
        ...
        const buffer = Buffer.from(data, "base64"); 
        Jimp.read(buffer, (err, res) => { if (err) throw new Error(err); res.quality(100).write('/app/' +req.body.doc_filename); });
